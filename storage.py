...
class RateLimitStore:
    """Persist the next time at which an X publication may start."""

    def __init__(self, path: Path) -> None:
        self.file = JsonFile(path, {"version": 1, "next_allowed_at": None})

    async def get_next_allowed_at(self) -> str | None:
        """Return the persisted ISO timestamp, when present."""

        data = await self.file.read()
        value = data.get("next_allowed_at")
        return str(value) if value else None

    async def set_next_allowed_at(self, value: str | None) -> None:
        """Persist the next allowed publication timestamp."""

        def mutate(data: dict[str, Any]) -> None:
            data["next_allowed_at"] = value
...
class PendingStore:
    ...
    async def clear(self) -> int:
        """Delete every pending job and return the number discarded."""
        ...
        return int(await self.file.mutate(mutate))
